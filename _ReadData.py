...
import json, pickle
import os, sys
from ReadData import welcome_message, translation
...
from convert2R import HPGe_efficiency_curve_generator
...
def extract_decay(dec_file):
    ...
    """
    half_life = uncertainties.core.Variable(np.clip(dec_file.half_life.n, 1E-23, None), dec_file.half_life.s) # increase the half-life to a non-zero value.
...
def condense_spectrum(dec_file):
    ...
    """
    count = uncertainties.core.Variable(0.0,0.0)
...
def deduce_daughter_from_mt(parent_atomic_number, parent_atomic_mass, mt):
    ...
    element_symbol = openmc.data.ATOMIC_SYMBOL[parent_atomic_number + translation[mt][0]]
    product_mass = str(parent_atomic_mass + translation[mt][1])
    if len(translation[mt])>2 and translation[mt][2]>0: # if it indicates an excited state
        excited_state = '_m'+str(translation[mt][2])
...
def extract_xs(parent_atomic_number, parent_atomic_mass, rx_file, tabulated=True):
    ...
    if len(rx_file.products)==0:
        name = deduce_daughter_from_mt(parent_atomic_number, parent_atomic_mass, rx_file.mt)+'-'+ str(rx_file.mt) # deduce_daughter_from_mt will return the ground state value
        appending_name_list.append(name)
        xs_list.append(detabulate(xs) if (not tabulated) else xs)
    else:
        for prod in rx_file.products:
            appending_name_list.append(prod.particle.replace('_e', '_m')+'-'+str(rx_file.mt))
...
def serialize_dict(dict_with_uncertainties):
    for key, val in dict_with_uncertainties.items():
        if isinstance(val, dict):
            serialize_dict(val)
        elif isinstance(val, (uncertainties.core.Variable, uncertainties.core.AffineScalarFunc)):
            dict_with_uncertainties[key] = str(val)
        else:
            pass
...
def clean_up_missing_records(xs_dict, decay_dict):
    ...
    for parent_product_mt in tqdm(list(xs_dict.keys())):
        if parent_product_mt.split('-')[1] not in decay_dict.keys():
            parent, product, mt = parent_product_mt.split('-')
            new_product = product.split('_')[0] # ignore the cases
            if new_product in decay_dict.keys():
                new_name = '-'.join([parent, new_product, mt])
                if new_name in xs_dict.keys():
                    ...
                    xs_dict[new_name] = xs_dict[parent_product_mt] # create new entry
...
if __name__=='__main__':
    ...
    assert os.path.exists(os.path.join(sys.argv[-1], 'integrated_apriori.csv')), "Output directory must already have integrated_apriori.csv"
        ...
        if repr(file).strip('<>').startswith('Incident-neutron'):
            ...
            for mt, rx in inc_f.reactions.items():
                ...
                if any([(mt in AMBIGUOUS_MT), (mt in FISSION_MTS), (301<=mt<=459)]):
                    continue # skip the cases of AMBIGUOUS_MT, fission mt, and heating information
    ...
    xs_dict = sort_and_trim_ordered_dict(xs_dict, trim_length=0)
