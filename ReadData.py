...
print(f"Loaded {len(endf_data)} different material files,\n") #should be a list
...
def get_names(endf_data):
    ...
    print()
    return gnd_name_list, eval_names, repr_name_list, sorting_names

gnd_name_list, eval_names, repr_name_list, sorting_names = get_names(endf_data)
...
def analyse_numbers_and_overlaps(endf_data):
...
analyse_numbers_and_overlaps(endf_data)
...
iso_dict = { gnd_name:[ _eval_obj for _eval_obj in endf_data if _eval_obj.gnd_name==gnd_name ] for gnd_name in gnd_name_list } # may lead to repeated data entry, but that's okay, because we're using a dictionary.
...
inc_r, inc_mt, all_mts, rcomp, resonances, kTs, dec_r = get_all_mt(iso_dict)
'''
...
Goal:
1. Read in database
2. Supply the list of isotopes
3. Get sigma and S_sigma
4. Get the decay radionuclide's spectrum
'''
...
1.2 convert to IncidentNeutron data and obtain the cross-section from .xs['0K'].x and .y
1.3 get the covariance data 
2. Get reaction products from the MF=8 file for each reaction; if not, deduce it.
3. Search for the relevant openmc.data.Decay spectrum
4. Add the melting point and fission etc. hazards flag to every isomer in a second pass
5. Return the interested isomer's all relevant xs.
'''